...
public class contributorsPipeline implements Pipeline {
    ...
    public static void openDB(String dbname) {
        ...
        try {
            githubRepo_stmt = con.prepareStatement("insert into developer values(?,?,?,?)");
        } catch (SQLException e) {
    ...
    public static void load_Githubrepo(Developer tmp) {
        try {
            if (con != null) {
                githubRepo_stmt.setLong(1, tmp.getId());
                githubRepo_stmt.setString(2, tmp.getRepoName());
                githubRepo_stmt.setString(3, tmp.getName());
                githubRepo_stmt.setInt(4, tmp.getCommit_num());
    ...
    public void process(ResultItems resultItems, Task task) {
        ...
        try {
            ...
            List<Developer> developers = resultItems.get("developers");
            developers.stream().forEach(o -> load_Githubrepo(o));
            try {
